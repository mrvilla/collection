...
cmdOptions.config =
	cmdOptions.config || `${veamsConfig.paths.config}/tasks/icons/webfont.config.js`;
	...
	webfontsGenerator(
		...
		(error, results) => {
			if (error) {
				console.error(
					chalk.red(
						'Webfont Generation Error :: Failed in generating the icons font.\n',
						error
					)
				);
				return;
...
function handleOutputFiles(result) {
	for (let i = 0; i < config.types.length; i++) {
		fs.outputFileSync(
			`${config.fontsPath}/${config.fontsName}.${config.types[i]}`,
			result[config.types[i]]
		);
	}

	return fs.outputFileSync(
		`${config.pluginOptions.dest}/${config.destFile}`,
		result.generateCss()
	);
}
...
globby(config.input)
	...
	.catch(err =>
		console.log(chalk.red('Webfont Generation Error :: Cannot read icons path.\n', err))
	);
